...
namespace Test01 {
    class ScoreCounter {
        ...
        private static IEnumerable<Student> ReadScore(string filePath) {
            var students = new List<Student>(); //売上データを格納する
            ...
            foreach (var line in lines) { //すべての行から一行ずつ取り出す
                ...
                students.Add(sale); //Saleインスタンスをコレクションに追加
            }
            return students;
        }
        ...
        public IDictionary<string, int> GetPerStudentScore() {
            ...
            foreach (Student student in _score) {
                if (dict.ContainsKey(student.Name))
                    dict[student.Name] += student.Score;　//店名が既に存在する
                else
                    dict[student.Name] = student.Score; //店名が存在しない (新規格納)
            }
            return dict;
        }
    }
